...
Annual_Income = st.number_input('Annual_Income', min_value=1000.0, format="%.2f", value=43391.96)
Monthly_Inhand_Salary = st.number_input('Monthly_Inhand_Salary', min_value=100.0, format="%.2f", value=3688.99)
Num_Bank_Accounts = st.number_input('Num_Bank_Accounts', min_value=1, value=2)
Num_Credit_Card = st.number_input('Num_Credit_Card', min_value=0, value=5)
Interest_Rate = st.number_input('Interest_Rate', min_value=1.0, format="%.2f", value=10.0)
Num_of_Loan = st.number_input('Num_of_Loan', min_value=0, value=0)
...
Num_of_Delayed_Payment = st.number_input('Num_of_Delayed_Payment', min_value=0, value=0)
Changed_Credit_Limit = st.number_input('Changed_Credit_Limit', min_value=0.0, format="%.2f", value=10.72)
Num_Credit_Inquiries = st.number_input('Num_Credit_Inquiries', min_value=0, value=0)
Credit_Mix = st.selectbox('Credit_Mix', ['Bad', 'Standard', 'Good'])
Outstanding_Debt = st.number_input('Outstanding_Debt', min_value=0.0, format="%.2f", value=1468.28)
Credit_Utilization_Ratio = st.number_input('Credit_Utilization_Ratio', min_value=20.0, format="%.2f", value=29.11)
Credit_History_Age = st.number_input('Credit_History_Age', min_value=0, value=284)
Payment_of_Min_Amount = st.selectbox('Payment_of_Min_Amount', ['Yes', 'No'])
Total_EMI_per_month = st.number_input('Total_EMI_per_month', min_value=0, value=0)
Amount_invested_monthly = st.number_input('Amount_invested_monthly', min_value=0.0, format="%.2f", value=47.438673)
Payment_Behaviour = st.selectbox('Payment_Behaviour', ['Low', 'Medium', 'High'])
Monthly_Balance = st.number_input('Monthly_Balance', min_value=-5000.0, format="%.2f", value=1554.930282)
